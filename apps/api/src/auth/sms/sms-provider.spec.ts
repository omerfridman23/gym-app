...
class RecordingProvider extends TextSmsProvider {
  readonly sendText = vi.fn<(_phone: string, _message: string) => Promise<void>>();
}
...
describe('TextSmsProvider.sendOtp', () => {
  it('delegates to sendText using the fixed Hebrew OTP prefix', async () => {
    ...
    await expect(provider.sendOtp('+972501234567', '012345')).resolves.toBeUndefined();
  ...
  it.each([
    ...
    null,
  ])('wraps delivery failure %j in the same generic exception', async (cause) => {
    const provider = new RecordingProvider();
    provider.sendText.mockRejectedValue(cause);

    const error = await provider.sendOtp('+972501234567', '123456').catch((value) => value);
